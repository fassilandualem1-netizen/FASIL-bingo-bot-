...
def welcome(m):
    ...
    if uid not in data["users"]: 
        data["users"][uid] = {"balance": 0.0, "name": m.from_user.first_name, "step": "", "sel_bid": None}
    
    # ድንገት balance ከሌለው 0 እንዲሆን ለማረጋገጥ
    if "balance" not in data["users"][uid]: data["users"][uid]["balance"] = 0.0
...
def handle_calls(c):
    ...
    # Ensure user exists
    if uid not in data["users"]: data["users"][uid] = {"balance": 0.0, "name": c.from_user.first_name, "step": "", "sel_bid": None}
    ...
    if c.data.startswith("start_sel_"):
        ...
        amt_val = float(amt)
        # ሂሳብ መደመር - በጥንቃቄ
        current_bal = float(data["users"][t_uid].get("balance", 0.0))
        data["users"][t_uid]["balance"] = current_bal + amt_val
        ...
        bot.send_message(ADMIN_ID, "✍️ እባክህ የደረሰኙን የብር መጠን ብቻ ጻፍ (ምሳሌ፡ 100)፦")
        ...
        if current_bal >= price:
            ...
            bot.answer_callback_query(c.id, f"❌ በቂ ብር የለዎትም! የመደብ ዋጋ {price} ETB ነው።", show_alert=True)

    # የተቀሩት የአድሚን ክፍሎች (Reset, Toggle etc) ባሉበት ቀጥለዋል...
...
def handle_msgs(m):
    uid = str(m.from_user.id)
    if uid not in data["users"]: data["users"][uid] = {"balance": 0.0, "name": m.from_user.first_name, "step": "", "sel_bid": None}
    u = data["users"][uid]
    ...
    if u['step'].startswith("INPUT_AMT_") and int(uid) == ADMIN_ID:
        ...
        bid = u.get("sel_bid")
        if not bid: bot.send_message(uid, "❌ መጀመሪያ ሰሌዳ ይምረጡ።"); return
        price = data["boards"][bid]["price"]
        bal = float(u.get("balance", 0.0))
        if bal >= price:
            b = data["boards"][bid]
            kb = telebot.types.InlineKeyboardMarkup(row_width=5)
            # ነጻ የሆኑ ቁጥሮችን ብቻ ማሳየት
            available = [telebot.types.InlineKeyboardButton(str(i), callback_data=f"n_{i}") for i in range(1, b["max"]+1) if str(i) not in b["slots"]]
            kb.add(*available)
            bot.send_message(uid, f"🔢 {b['name']} ቁጥር ይምረጡ (የአንዱ ዋጋ {price} ETB)፦", reply_markup=kb)
        else: 
            bot.send_message(uid, f"❌ በቂ ብር የለዎትም! ለመምረጥ ቢያንስ {price} ETB ያስፈልጋል።")
        ...
        bid = u.get("sel_bid")
        if not bid: 
            bot.send_message(uid, "⚠️ መጀመሪያ ሰሌዳ ይምረጡ (/start)")
            return
        
        # ደረሰኙ ላይ ያለውን ብር ለመለየት
        ...
        board_name = data["boards"][bid]["name"]
        if m.content_type == 'photo': 
            bot.send_photo(ADMIN_ID, m.photo[-1].file_id, caption=f"📩 ፎቶ ከ {m.from_user.first_name}\n{board_name}\nየተላከው ብር፡ {sent_amount}", reply_markup=kb)
        else: 
