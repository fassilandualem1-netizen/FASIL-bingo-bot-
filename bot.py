...
@app.route('/')
def home(): return "Fasil Lottery v13 - Fixed & Decorated! 🎰"
...
def generate_board_text():
    ...
    for i in range(1, 101):
        ...
        if i % 4 == 0: text += "\n"
    text += "\n━━━━━━━━━━━━━━━━━━━━\n"
    text += "⏳ **Next Waiting: ቀጣይ ዕጣ በቅርቡ ይጠብቁ!**\n"
...
def handle_callbacks(call):
    ...
    if call.data.startswith("win_"):
        ...
        bot.answer_callback_query(call.id, "♻️ Game Reset Done!")
        ...
        bot.answer_callback_query(call.id, f"✅ ቁጥር {num} ተመርጧል! Next Waiting!")
...
def handle_private(message):
    ...
    if message.text == "🛠 Admin Panel" and uid == ADMIN_ID:
        ...
        bot.send_message(ADMIN_ID, "🛠 **የአድሚን መቆጣጠሪያ ፓነል**", reply_markup=markup)

    elif u_data['step'] == 'SET_PRICE' and uid == ADMIN_ID:
        try:
            game_data['price'] = int(message.text)
            u_data['step'] = ''
            bot.send_message(ADMIN_ID, f"✅ የመደብ ዋጋ ወደ {message.text} ETB ተቀይሯል።")
        except: bot.send_message(ADMIN_ID, "⚠️ እባክዎ በቁጥር ብቻ ይጻፉ።")
        ...
        msg = (f"👋 **እንኳን ወደ ፋሲል ዕጣ በደህና መጡ!**\n\n"
               f"ጥራት ያለው እና ታማኝ አገልግሎት እንሰጣለን።\n\n"
               f"🏦 **የመክፈያ አካውንቶች (ለመቅዳት ይጫኑ):**\n\n"
               f"🔸 **CBE:** `1000572166645` (ፋሲል)\n"
               f"🔸 **Telebirr:** `0960309130` (ፋሲል)\n\n"
               f"💵 **የአንድ እጣ ዋጋ:** `{game_data['price']} ብር`\n\n"
               f"📍 መመሪያ፡ ከላይ ባሉት አካውንቶች ክፍያ ፈጽመው ደረሰኙን እዚህ ይላኩ።")
