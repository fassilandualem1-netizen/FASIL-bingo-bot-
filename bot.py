...
def handle_calls(c):
    ...
    if c.data.startswith("ok_"):
        ...
        bot.send_message(t_uid, f"✅ ደረሰኝዎ ጸድቋል! {tks} እጣ ተሰጥቶዎታል። አሁን ስምዎን ይጻፉ፦")
        bot.delete_message(ADMIN_ID, c.message.message_id)

    elif c.data.startswith("no_"):
        bot.send_message(c.data.split("_")[1], "❌ ይቅርታ የላኩት ደረሰኝ ስህተት ነው ወይም የቆየ ስለሆነ ውድቅ ተደርጓል።")
        bot.delete_message(ADMIN_ID, c.message.message_id)
...
def handle_private(m):
    ...
    if m.text == "/start":
        ...
        except: bot.send_message(uid, "❌ ስህተት! ቁጥሮችን በኮማ ይጻፉ።")

    elif u['step'] == "ASK_NAME":
        upd_u(uid, {"display_name": m.text, "step": ""}); bot.send_message(uid, "✅ ተመዝግቧል! አሁን '🕹 ቁጥር ምረጥ' የሚለውን ይጫኑ።")
        ...
        if u['tickets'] <= 0: bot.send_message(uid, "❌ መጀመሪያ እጣ ይግዙ።")
        else:
